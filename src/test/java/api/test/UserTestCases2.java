...
import api.endpoints.UserEnpoints2;
...
public class UserTestCases2 {
	...
	public void testPostUser(){
		logger.info("\nCreating user\n");
		Response response = UserEnpoints2.createUser(userPayload); // here we are passing the payload of user data created in above method to create a user
	...
	public void testGetUserByName() {
		System.out.println("\nUser is fetched\n");
		Response response=UserEnpoints2.getUser(userPayload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.statusCode(), 200);
	}
	...
	public void testPutUserByName() {	
		...
		Response response = UserEnpoints2.updateUser(userPayload.getUsername(), userPayload);// user details is updated with above changes21	1
		response.then().log().all();
		Assert.assertEquals(response.statusCode(), 200);

		//To verify the user details is updated or not
		Response afterUpdateResponse=UserEnpoints2.getUser(userPayload.getUsername());
	...
	public void testDeleteUserByName() {
		System.out.println("\n User is deleted\n");
		Response response = UserEnpoints2.deleteUser(userPayload.getUsername());
